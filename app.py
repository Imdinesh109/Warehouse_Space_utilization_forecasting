...
if execute_analysis:
    ...
    try:
        ...
        future_datetime = base_datetime + datetime.timedelta(
        ...
        scaled_ohe = pd.DataFrame(
            production_ohe.transform(
                df_input[low_cardinality_categoricals]
            ),
            columns=low_cardinality_headers
        )

        scaled_te = production_te.transform(
            df_input[high_cardinality_categorical]
        )
        ...
        computed_available_m3 = (
            zone_capacity_m3 - computed_occupied_m3
        )
